...
constexpr uint32_t kResponseTimeMs = 200UL;   // max wait after last request byte
...
void Rs485RtuMaster::begin() {
    if (rtuPinsOk(PIN_LEFT_RS485_RX, PIN_LEFT_RS485_TX)) {
        pinMode(PIN_LEFT_RS485_RTS, OUTPUT); digitalWrite(PIN_LEFT_RS485_RTS, rxIdleLevel(RtuBus::LEFT));
    } else {
        Serial.println("[RTU] WARNING: left bus pins conflict with OPI PSRAM - left bus disabled");
    }
    pinMode(PIN_RIGHT_RS485_RTS, OUTPUT); digitalWrite(PIN_RIGHT_RS485_RTS, rxIdleLevel(RtuBus::RIGHT));
...
void Rs485RtuMaster::setTx(RtuBus bus) {
    if (bus == RtuBus::LEFT) {
        digitalWrite(PIN_LEFT_RS485_RTS, txActiveLevel(bus));
        Serial.printf("[RTU] DE GPIO%d -> %s (TX)\n", PIN_LEFT_RS485_RTS, txActiveLevel(bus) ? "HIGH" : "LOW");
    } else {
        digitalWrite(PIN_RIGHT_RS485_RTS, txActiveLevel(bus));
        Serial.printf("[RTU] DE GPIO%d -> %s (TX)\n", PIN_RIGHT_RS485_RTS, txActiveLevel(bus) ? "HIGH" : "LOW");
    }
...
void Rs485RtuMaster::setRx(RtuBus bus) {
    if (bus == RtuBus::LEFT) {
        digitalWrite(PIN_LEFT_RS485_RTS, rxIdleLevel(bus));
    } else {
        digitalWrite(PIN_RIGHT_RS485_RTS, rxIdleLevel(bus));
    }
    delayMicroseconds(100);
}
                                       ...
                                       size_t expectedLen)
{
    ...
    rtuSerial.flush();
    // On this platform the driver can return from flush() before the line has fully settled.
    // Hold DE high long enough for the complete request frame to clear the transceiver.
    delay(kPostFlushTxHoldMs);
    setRx(_activeBus);

    // Receive with timeout
    size_t received = 0;
    const uint32_t deadline = millis() + kResponseTimeMs;
    while (millis() < deadline && received < expectedLen && received < respMaxLen) {
