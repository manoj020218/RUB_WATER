...
function shapeTelemetry(record) {
  ...
  return {
    ...record,
    hardware_id: record.hardware_id || null,
    mqtt_route_id: record.mqtt_route_id || null,
    mqtt_topic_base: record.mqtt_topic_base || null,
...
function writeIdentityAudit({ device, eventType, details }) {
  if (!device || !device.location_id) {
    return;
  }
  auditService.writeAuditLog({
...
function reconcileDeviceIdentity(device, payload, source) {
  if (!device) {
    throw notFound('Device not found');
  }

  const reportedHardwareId = deviceRepository.normalizeHardwareId(payload.hardware_id || '');
  const reportedDeviceId = deviceRepository.normalizeDeviceId(payload.device_id || '');
  let resolved = device;

  if (reportedHardwareId && deviceRepository.isHardwareRebindAllowed(resolved, reportedHardwareId)) {
    const previousHardwareId = resolved.hardware_id || null;
    resolved = deviceRepository.bindHardware(resolved._id, reportedHardwareId, {
      reason: `${source}_reported_identity`,
      reportedDeviceId
    });
    if (previousHardwareId && previousHardwareId !== reportedHardwareId) {
      writeIdentityAudit({
        device: resolved,
        eventType: 'DEVICE_HARDWARE_REBOUND',
        details: {
          previous_hardware_id: previousHardwareId,
          hardware_id: reportedHardwareId,
          source
        }
      });
      resolved = transitionDeviceLifecycleForReplacement(
        resolved,
        'PENDING_VERIFICATION',
        `Replacement hardware ${reportedHardwareId} detected from ${source}`,
        source,
        {
          previous_hardware_id: previousHardwareId,
          hardware_id: reportedHardwareId
  ...
  } else if (reportedHardwareId && resolved.hardware_id && resolved.hardware_id !== reportedHardwareId) {
    writeIdentityAudit({
      device: resolved,
      eventType: 'DEVICE_HARDWARE_IDENTITY_CONFLICT',
      details: {
        expected_hardware_id: resolved.hardware_id,
        reported_hardware_id: reportedHardwareId,
        source
      }
    });
  }

  if (reportedDeviceId && reportedDeviceId !== resolved._id) {
    const existing = deviceRepository.findById(reportedDeviceId);
    if (!existing || existing._id === resolved._id) {
      const previousDeviceId = resolved._id;
      resolved = deviceRepository.renameDevice(resolved._id, reportedDeviceId, {
        reason: `${source}_reported_device_id`
      });
      writeIdentityAudit({
        device: resolved,
        eventType: 'DEVICE_ID_RENAMED_BY_DEVICE',
        details: {
          previous_device_id: previousDeviceId,
          device_id: reportedDeviceId,
          hardware_id: resolved.hardware_id || reportedHardwareId || null,
          source
        }
      });
    } else {
      writeIdentityAudit({
        device: resolved,
        eventType: 'DEVICE_ID_RENAME_CONFLICT',
        details: {
          current_device_id: resolved._id,
          reported_device_id: reportedDeviceId,
          conflicting_device_id: existing._id,
          hardware_id: resolved.hardware_id || reportedHardwareId || null,
          source
        }
      });
...
function resolveDeviceFromPayload(payload, source) {
  const device = deviceRepository.resolveByIdentity({
    deviceId: payload.device_id,
    hardwareId: payload.hardware_id,
    routeId: payload.mqtt_route_id
  });
  if (!device) {
    throw notFound('Device not found');
  }
  return reconcileDeviceIdentity(device, payload, source);
}

function transitionDeviceLifecycleForReplacement(device, nextStatus, reason, source, details = {}) {
  if (!device || !device.location_id) {
    return device;
  }
  const previousStatus = String(device.operational_status || 'ACTIVE').toUpperCase();
  if (previousStatus === nextStatus && device.lifecycle_note === reason) {
    return device;
  }
  const updated = deviceRepository.updateOperationalStatus(device._id, nextStatus, reason);
  writeIdentityAudit({
    device: updated,
    eventType: 'DEVICE_REPLACEMENT_LIFECYCLE_AUTO',
    details: {
...
function maybeActivateReplacementHardware({ deviceId, locationId, reportedConfig, source, now }) {
  ...
  const updated = deviceRepository.updateOperationalStatus(
    deviceId,
    'ACTIVE',
    `Replacement hardware verified from ${source}`
  );
  auditService.writeAuditLog({
    locationId,
  ...
  publishRealtime('DEVICE_LIFECYCLE_CHANGED', {
    ...
    previous_status: currentStatus,
    next_status: 'ACTIVE',
    reason: `Replacement hardware verified from ${source}`
  });
}

function ingestTelemetry(payload) {
  const device = resolveDeviceFromPayload(payload, 'telemetry');

  const resolvedPayload = {
    ...payload,
    device_id: device._id,
    hardware_id: payload.hardware_id || device.hardware_id || null,
    mqtt_route_id: payload.mqtt_route_id || device.mqtt_route_id || null,
    mqtt_topic_base: payload.mqtt_topic_base || device.mqtt_topic_base || null,
    location_id: payload.location_id || device.location_id || null
  };
...
function ingestEvent(payload) {
  const device = resolveDeviceFromPayload(payload, 'event');
  ...
  deviceRepository.updateRuntime(event.device_id, {
    ...
    hardware_id: event.hardware_id || device.hardware_id || null,
    last_reported_device_id: deviceRepository.normalizeDeviceId(payload.device_id || event.device_id),
    mqtt_route_id: payload.mqtt_route_id || device.mqtt_route_id || device._id,
    mqtt_topic_base: payload.mqtt_topic_base || device.mqtt_topic_base || null,
...
function ingestHeartbeat(payload = {}, topicDeviceId = null) {
  const device = resolveDeviceFromPayload({
    ...payload,
    mqtt_route_id: payload.mqtt_route_id || topicDeviceId || null
  }, 'heartbeat');

  const timestamp = toIso(payload.timestamp || payload.timestamp_iso || null);
  const online = payload.online !== false;
  const status = online ? 'ONLINE' : 'OFFLINE';
  const hardwareId = payload.hardware_id || device.hardware_id || null;
  const routeId = payload.mqtt_route_id || device.mqtt_route_id || device._id;
  const topicBase = payload.mqtt_topic_base || device.mqtt_topic_base || null;

  deviceRepository.updateRuntime(device._id, {
    status,
    last_seen: timestamp,
    ...
    mqtt_topic_base: topicBase,
    last_reported_device_id: deviceRepository.normalizeDeviceId(payload.device_id || device._id),
    last_current_config_version: Number(payload.current_config_version ?? 0) || null
  });
...
function getLocationDashboard(locationId) {
  ...
  return {
    ...
    device: device ? {
      ...
      mqtt_route_id: device.mqtt_route_id || null,
      last_reported_device_id: device.last_reported_device_id || null,
      status: deviceStatus,
